...
import org.jboss.arquillian.iphone.spi.event.IPhoneConfigured;
...
public class ApplicationLauncherRegistrar {
    ...
    @Inject
    @SuiteScoped
    private InstanceProducer<IPhoneConfiguration> configuration;
    @Inject
    private Event<IPhoneConfigured> configured;
    @Inject
    private Event<IPhoneReady> ready;
    ...
    public void configure(@Observes BeforeSuite event, ArquillianDescriptor descriptor) {
        configuration.set(new IPhoneConfiguration().configure(descriptor, Default.class));
        configured.fire(new IPhoneConfigured());
    }

    public void register(@Observes IPhoneConfigured event) throws IOException {
        WaxSim waxSim;
